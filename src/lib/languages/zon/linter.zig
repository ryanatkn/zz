            ...
            if (std.mem.eql(u8, child.rule_name, "field_assignment") and child.children.len >= 1) {
                ...
                if (seen_keys.get(key_name)) |previous_span| {
                    ...
                    try self.addDiagnosticOwned(
                        "no-duplicate-keys",
                        message,
                        field_name_node.start_position,
                        field_name_node.end_position,
                        .@"error"
                    );
                } else {
            ...
            if (std.mem.eql(u8, child.rule_name, "field_assignment") and child.children.len >= 2) {
                ...
                if (std.mem.eql(u8, field_name, "name")) {
                    ...
                    if (!std.mem.eql(u8, value_node.rule_name, "string_literal") and
                        ...
                        try self.addDiagnostic(
                            "invalid-field-type",
                            "Package name must be a string or identifier",
            ...
            if (std.mem.eql(u8, child.rule_name, "field_assignment") and child.children.len >= 2) {
                ...
                if (std.mem.eql(u8, field_name, "base_patterns")) {
                    ...
                    if (!std.mem.eql(u8, value_node.rule_name, "string_literal")) {
                        try self.addDiagnostic(
                            "invalid-field-type",
                            "base_patterns should be a string",
                            value_node.start_position,
                            value_node.end_position,
                            .warning
                        );
                    ...
                    if (!std.mem.eql(u8, value_node.rule_name, "array")) {
                        try self.addDiagnostic(
                            "invalid-field-type",
                            "ignored_patterns should be an array",
                            value_node.start_position,
                            value_node.end_position,
                            .warning
                        );
                    ...
                    if (!std.mem.eql(u8, value_node.rule_name, "boolean_literal")) {
                        try self.addDiagnostic(
                            "invalid-field-type",
                            "respect_gitignore should be a boolean",
                            value_node.start_position,
                            value_node.end_position,
                            .warning
                        );
