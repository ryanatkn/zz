...
pub const SemanticAnalysis = struct {
    ...
    pub const CodeSummarizer = struct {
        ...
        pub fn generateCodebaseSummary(self: *CodeSummarizer, relevance_scores: []const FileRelevance, max_files: u32, max_chars: u32) ![]u8 {
        ...
        fn generateFileSummary(self: *CodeSummarizer, relevance: FileRelevance) ![]u8 {
            ...
            // File header
            try summary.appendSlice(try std.fmt.allocPrint(self.allocator, "### {s} (Score: {d:.2})\n", .{ path_utils.basename(relevance.file_path), relevance.totalScore() }));

            // Role and basic info
            try summary.appendSlice(try std.fmt.allocPrint(self.allocator, "**Role**: {s} | **Path**: `{s}`\n\n", .{ @tagName(relevance.role), relevance.file_path }));
    ...
    pub const ContextSelector = struct {
        ...
        pub fn selectFilesForQuery(allocator: std.mem.Allocator, relevance_scores: []const FileRelevance, query_intent: QueryIntent, token_budget: u32) ![][]const u8 {
