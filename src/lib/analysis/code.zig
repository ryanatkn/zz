...
pub const CodeAnalysis = struct {
    ...
    pub const CallGraphBuilder = struct {
            ...
            while (lines.next()) |line| {
                ...
                if (current_function) |caller| {
                    ...
                    while (call_iter.next()) |callee| {
                        try self.addCallRelationship(caller, callee, file_path, line_number, self.classifyCallType(line, callee));
                    }
                }
            }
        ...
        fn addCallRelationship(self: *CallGraphBuilder, caller: []const u8, callee: []const u8, file_path: []const u8, line_number: u32, call_type: CallRelationship.CallType) !void {
